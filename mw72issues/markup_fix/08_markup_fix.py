"""
Markup fixer + audit for mw72.txt (MW72).
Counterpart of PWG#175, PWK#113, and siblings.

MW72-specific notes: 1 lead + 12 trail in <nsi>, 1 lead + 11 trail in <lang>.

Inputs: ../../../csl-orig/v02/mw72/mw72.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "mw72" / "mw72.txt", HERE / "mw72.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "mw72_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(nsi|lang|F|s)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for mw72.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"MW72 markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against mw72.txt.\n")
        f.write(f"Paired tags: <nsi>, <lang>, <F>, <s>\n")
        f.write(f"Notes: 1 lead + 12 trail in <nsi>, 1 lead + 11 trail in <lang>.\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <nsi>, <lang>, <F>, <s>\n\n")
